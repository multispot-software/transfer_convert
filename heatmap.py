...
def heatmap48(values=None, title=None, vert=False, figsize=(14, 4), ax=None, skip_ch=None, **kwargs):   
    if values is None:
        values = np.arange(48)
    ...
    if values.shape != (4, 12):
        raise ValueError('Input data has wrong shape (%s). If must be (48,), (12, 4) or (4, 12).' % values.shape)
